...
let records_map = ref RecordMap.empty
...
let lookup_records_map_opt bt =
  match bt with BT.Record members -> RecordMap.find_opt members !records_map | _ -> None
...
let get_records () = RecordMap.bindings !records_map

(* TODO: Complete *)
let rec base_type ?pred_sym:(_ = None) cn_typ =
...
let bt_to_ail_ctype ?(pred_sym = None) t = base_type ~pred_sym (bt_to_cn_base_type t)
...
let rec expr_aux
  ...
  match term_ with
  | Const c ->
    let ail_expr, is_unit = const c basetype in
    ...
    let ail_expr_ =
      ...
      | Some (sym2, cn_const) ->
        if CF.Symbol.equal_sym sym sym2 then (
          let ail_const, _ = const cn_const basetype in
          ail_const)
        else
          A.(AilEident sym)
  ...
  | Binop (bop, t1, t2) ->
    let b1, s1, e1 = expr_aux const_prop pred_name dts globals t1 PassBack in
    let b2, s2, e2 = expr_aux const_prop pred_name dts globals t2 PassBack in
    let annot = binop (IT.get_bt t1) (IT.get_bt t2) bop in
    let str =
      match annot with
      | Some str -> str
  ...
  | Unop (op, t) ->
    let b, s, e = expr_aux const_prop pred_name dts globals t PassBack in
    let annot = unop (IT.get_bt t) op in
    let str =
      match annot with
      | Some str -> str
  ...
  | ITE (t1, t2, t3) ->
    ...
    in
    let b2, s2 = expr_aux const_prop pred_name dts globals t2 (AssignVar result_sym) in
    let b3, s3 = expr_aux const_prop pred_name dts globals t3 (AssignVar result_sym) in
    let ite_stat =
      A.(
        AilSif
  ...
  | EachI ((r_start, (sym, bt'), r_end), t) ->
    ...
    *)
    ...
    let _, _, start_int_const =
      expr_aux const_prop pred_name dts globals start_const_it PassBack
    in
    let while_cond_it =
      IT.(IT (Binop (LT, incr_var, end_const_it), bt', Cerb_location.unknown))
    in
    let _, _, while_cond =
      expr_aux const_prop pred_name dts globals while_cond_it PassBack
    in
    let translated_t = expr_aux const_prop pred_name dts globals t PassBack in
    let bs, ss, e =
  ...
  | StructUpdate ((struct_term, m), new_val) ->
    ...
    (match tag_def with
     | C.StructDef (members, _) ->
       let b1, s1, e1 = expr_aux const_prop pred_name dts globals struct_term PassBack in
  ...
  | MemberShift (it, tag, member) ->
    ...
    let bs, ss, e = expr_aux const_prop pred_name dts globals it PassBack in
    let ail_fcall =
      A.(
        AilEcall
  ...
  | ArrayShift { base; ct; index } ->
    let b1, s1, e1 = expr_aux const_prop pred_name dts globals base PassBack in
    let b2, s2, e2 = expr_aux const_prop pred_name dts globals index PassBack in
  ...
  | Head xs ->
    let b, s, e = expr_aux const_prop pred_name dts globals xs PassBack in
  ...
  | WrapI (_ct, t) -> expr_aux const_prop pred_name dts globals t d
  | MapConst (_bt, _t) -> failwith (__LOC__ ^ ": TODO MapConst")
  | MapSet (m, key, value) ->
    let b1, s1, e1 = expr_aux const_prop pred_name dts globals m PassBack in
    let key_term =
    ...
    in
    let b2, s2, e2 = expr_aux const_prop pred_name dts globals key_term PassBack in
    let b3, s3, e3 = expr_aux const_prop pred_name dts globals value PassBack in
  ...
  | Apply (sym, ts) ->
    let bs_ss_es =
      List.map (fun e -> expr_aux const_prop pred_name dts globals e PassBack) ts
    in
  ...
  | Let ((var, t1), body) ->
    ...
    prefix
      ...
      (expr_aux const_prop pred_name dts globals body d)
  | Match (t, ps) ->
    (* PATTERN COMPILER *)
    ...
    let rec translate
      ...
      | [] ->
        (match cases with
         | ([], t) :: _rest ->
           (match d with
            | Assert loc -> expr_aux const_prop pred_name dts globals t (Assert loc)
            | Return -> expr_aux const_prop pred_name dts globals t Return
            | AssignVar x -> expr_aux const_prop pred_name dts globals t (AssignVar x)
            | PassBack ->
              expr_aux
                const_prop
                pred_name
        ...
        if List.for_all leading_wildcard cases then (
          ...
          | BT.Datatype sym ->
            ...
            (match cn_dt with
             ...
             | dt :: _ ->
               let b1, s1, e1 = expr_aux const_prop pred_name dts globals term PassBack in
...
let expr
  ...
  =
  fun dts globals cn_expr d -> expr_aux None None dts globals cn_expr d
...
let expr_toplevel
  ...
  : A.bindings
    ...
    * CF.GenTypes.genTypeCategory A.expression
  =
  expr_aux None pred_sym_opt dts globals it PassBack


let expr_with_pred_name
  : type a.
    Sym.t option ->
    _ CF.Cn.cn_datatype list ->
    (C.union_tag * C.ctype) list ->
  ...
  expr_aux None pred_name_opt dts globals cn_expr d
...
let resource
  ...
  | Request.P p ->
    ...
    let b, s, e = expr dts globals p.pointer PassBack in
    let rhs, bs, ss =
      match p.name with
      | Owned (sct, _) ->
        ...
        let fn_call_it =
          IT.IT
            ( Apply (Sym.fresh owned_fn_name, [ p.pointer; enum_val_get ]),
              ...
              Cerb_location.unknown )
        in
        let bs', ss', e' = expr dts globals fn_call_it PassBack in
  ...
  | Request.Q q ->
    ...
    *)
    let b1, s1, _e1 = expr dts globals q.pointer PassBack in
    (*
    ...
    *)
    ...
    let _, _, e_start = expr dts globals start_expr PassBack in
    let _, _, while_cond_expr = expr dts globals while_loop_cond PassBack in
    let _, _, if_cond_expr = expr dts globals q.permission PassBack in
    let cn_integer_ptr_ctype = bt_to_ail_ctype i_bt in
    let b2, s2, _e2 = expr dts globals q.permission PassBack in
    let b3, s3, _e3 =
      expr dts globals (IT.sizeOf_ q.step Cerb_location.unknown) PassBack
    in
    ...
    in
    let b4, s4, e4 = expr dts globals value_it PassBack in
...
let logical_constraint_aux
  ...
  match lc with
  | LogicalConstraints.T it -> expr dts globals it d
    ...
    (match IT.get_term t with
     ...
     | _ ->
       ...
       *)
       ...
       let _, _, e_start = expr dts globals start_expr PassBack in
       let _, _, while_cond_expr = expr dts globals while_loop_cond PassBack in
       let _, _, if_cond_expr = expr dts globals cond_it PassBack in
       let t_translated = expr dts globals t PassBack in
       let bs, ss, e =
         gen_bool_while_loop
           sym
           bt
...
let function_
  ...
  let bs, ail_func_body_opt =
    ...
    | Def it | Rec_Def it ->
      let bs, ss = expr_with_pred_name (Some fn_sym) cn_datatypes [] it Return in
...
let rec logical_argument_type ?(is_toplevel = true) dts pred_sym_opt globals preds
  = function
  | LAT.Define ((name, it), _info, lat) ->
    ...
    let b1, s1 = expr_with_pred_name pred_sym_opt dts globals it (AssignVar name) in
  ...
  | LAT.Constraint (lc, (loc, _str_opt), lat) ->
    ...
    (b1 @ b2, ss @ s2)
  | LAT.I it ->
    let bs, ss = expr_with_pred_name pred_sym_opt dts globals it Return in
    (bs, ss)


let predicate dts globals preds cn_preds (pred_sym, (rp_def : Definition.Predicate.t)) =
    ...
    | c :: cs ->
      ...
      (match c.guard with
       | IT (Const (Bool true), _, _) ->
         ...
         (bs @ bs'', ss @ ss'')
       | _ ->
         let _, _, e = expr_with_pred_name (Some pred_sym) dts [] c.guard PassBack in
         let bs'', ss'' = clause_translate cs in
...
let predicates preds dts globals cn_preds
  : ((Locations.t * A.sigma_declaration)
    * CF.GenTypes.genTypeCategory A.sigma_function_definition)
      list
    * A.sigma_tag_definition option list
  =
  List.split (List.map (predicate dts globals preds cn_preds) preds)
...
let rec post_aux dts globals preds = function
  | LRT.Define ((name, it), (_loc, _), t) ->
    ...
    let b1, s1 = expr dts globals it (AssignVar new_name) in
...
let post (*loc*) dts globals preds (ReturnTypes.Computational (_bound, _oinfo, t)) =
  let bs, ss = post_aux dts globals preds t in
...
let cnstatement
  ...
  | Assert lc -> (logical_constraint_aux dts globals d lc, false)
  | Inline _ -> failwith "TODO Inline"
  | Print _t -> (default_res_for_dest, true)


let rec cnprog_aux dts globals = function
  | Cnprog.Let (_loc, (name, { ct; pointer }), prog) ->
    let b1, s, e = expr dts globals pointer PassBack in
    ...
    in
    let (b2, ss), no_op = cnprog_aux dts globals prog in
    if no_op then
      (([], []), true)
    else
...
let cnprog dts globals cn_prog =
  let (bs, ss), _ = cnprog_aux dts globals cn_prog in
  (bs, ss)
...
let rec lat_internal_loop ?(is_toplevel = true) dts globals preds = function
  | LAT.Define ((name, it), _info, lat) ->
    let ctype = bt_to_ail_ctype (IT.get_bt it) in
    let binding = create_binding name ctype in
    let decl = A.(AilSdeclaration [ (name, None) ]) in
    let b1, s1 = expr dts globals it (AssignVar name) in
  ...
  | LAT.I ss ->
    let ail_statements =
      List.map (fun stat_pair -> statements dts globals stat_pair) ss
    in
...
let rec loop_inv_aux dts globals preds (contains_user_spec, cond_loc, loop_loc, at) =
  ...
  | AT.Computational ((sym, bt), _, at') ->
    ...
    let (_, (cond_bs, cond_ss)), (_, (loop_bs, loop_ss)) =
      loop_inv_aux dts globals preds subst_loop
    in
  ...
  | L lat ->
    ...
    in
    let bs, ss = lat_internal_loop dts globals preds lat in
...
let loop_inv
  ...
  if contains_user_spec then (
    let (_, (cond_bs, cond_ss)), (_, loop_bs_and_ss) =
      loop_inv_aux dts globals preds loop
    in
...
let rec lat_2
  ...
  | LAT.I (return_type, (stats, loop)) ->
    ...
    let ail_statements =
      List.map (fun stat_pair -> statements dts globals stat_pair) stats
    in
    let ail_loop_invariants =
      List.map (loop_inv dts globals preds with_loop_leak_checks) loop
    in
    let ail_loop_invariants = List.filter_map Fun.id ail_loop_invariants in
    let post_bs, post_ss = post dts globals preds return_type in
...
let assume_resource sym dts globals (preds : (Sym.t * Definition.Predicate.t) list) loc =
...
let rec assume_lat dts pred_sym_opt globals preds = function
  | LAT.Define ((name, it), _info, lat) ->
    let ctype = bt_to_ail_ctype (IT.get_bt it) in
    let binding = create_binding name ctype in
    let decl = A.(AilSdeclaration [ (name, None) ]) in
    let b1, s1 = expr_with_pred_name pred_sym_opt dts globals it (AssignVar name) in
    let b2, s2 = assume_lat dts pred_sym_opt globals preds lat in
    (b1 @ b2 @ [ binding ], (decl :: s1) @ s2)
  | LAT.Resource ((name, (ret, _bt)), (loc, _str_opt), lat) ->
    let b1, s1 = assume_resource name dts globals preds loc ret in
    let b2, s2 = assume_lat dts pred_sym_opt globals preds lat in
    (b1 @ b2, s1 @ s2)
...
let assume_predicate (pred_sym, (rp_def : Definition.Predicate.t)) dts globals preds =
  let ret_type = bt_to_ail_ctype ~pred_sym:(Some pred_sym) rp_def.oarg_bt in
  let rec clause_translate (clauses : Definition.Clause.t list) =
    match clauses with
    | [] -> ([], [])
    | c :: cs ->
      let bs, ss = assume_lat dts (Some pred_sym) globals preds c.packing_ft in
      (match c.guard with
       | IT (Const (Bool true), _, _) ->
         let bs'', ss'' = clause_translate cs in
         (bs @ bs'', ss @ ss'')
...
let rec assume_predicates pred_def_list dts globals preds
  : (A.sigma_declaration * CF.GenTypes.genTypeCategory A.sigma_function_definition) list
  =
  match pred_def_list with
  | [] -> []
  | p :: ps ->
    let d = assume_predicate p dts globals preds in
    let ds = assume_predicates ps dts globals preds in
    d :: ds


let rec assume_lat_2 dts pred_sym_opt globals preds = function
  | LAT.Define ((name, it), _info, lat) ->
    let ctype = bt_to_ail_ctype (IT.get_bt it) in
    let binding = create_binding name ctype in
    let decl = A.(AilSdeclaration [ (name, None) ]) in
    let b1, s1 = expr_with_pred_name pred_sym_opt dts globals it (AssignVar name) in
    let b2, s2 = assume_lat_2 dts pred_sym_opt globals preds lat in
    (b1 @ b2 @ [ binding ], (decl :: s1) @ s2)
  | LAT.Resource ((name, (ret, _bt)), (loc, _str_opt), lat) ->
    let b1, s1 = assume_resource name dts globals preds loc ret in
    let b2, s2 = assume_lat_2 dts pred_sym_opt globals preds lat in
...
let assume_pre dts sym args globals preds lat
  : A.sigma_declaration * CF.GenTypes.genTypeCategory A.sigma_function_definition
  =
  let open Option in
  ...
  in
  (* Generate function *)
  let bs', ss' = assume_lat_2 dts (Some sym) globals preds lat in
